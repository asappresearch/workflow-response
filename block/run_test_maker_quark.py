...
quark_paths = [  "./outputs/08-03-2023_17:15:45/model/"]
...
for quark_path in quark_paths:
    ...
    policy = Policy(model_name=quark_path, temperature=1.0, device=device,
                    reward_cond=True, tree_tokens=tree_tokens, oracle=True)
    test_dataset = PromptDataset(dataset_type="a2", path="data/wc_seed_one_convo.json", split="dev", data_len=2000, oracle=True)
