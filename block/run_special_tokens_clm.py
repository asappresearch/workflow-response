...
def example_process(dataset_type: str, datum):
    ...
    for turn in turns:
        ...
        #         string += ACTION  + " " +button + " " + ", ".join(slot).strip() +  " " + ACTION_END + " "
        # elif speaker == "system":
        #     if dataset_type == "b1" or dataset_type == "b2":
        #         string += RESPONSE + " " + text + " " + RESPONSE_END + " "
        #     elif "future" in dataset_type:
        #         workflow = turn["workflow_action"]
        #         if workflow != [None]:
        ...
        #     print("impossib")
        #     exit()
    string = string.strip() +  CONTEXT_END
    return string.strip()

def process_with_tokens(text: str):
    """
    ################
    CONTEXT = "<|context|>"
    ...
    # text = text.replace("")
    utts = text.split("\n")
    utts = [ x.strip() for x in utts if x.strip() !=""]
    processed = CONTEXT + " "
    for i, utt in enumerate(utts):
        """TODO: FIX THIS PART: to correctly insert end of context
        problem: b1 / b2 end of context not inserted
        problem: for oracle, context end is inserted multiple times
        """        

        if "system:" in utt:
            if i == len(utts) - 1:
                processed += RESPONSE  + utt.replace("system:", "") + " " + RESPONSE_END + " "
            else:
                processed += utt.replace("system:", SYSTEM) + " "
        elif "user:" in utt:
            processed += utt.replace("user:", USER) + " "
        elif "wf-action:" in utt:
            #processed += CONTEXT_END + " " + utt.replace("wf-action:", WORKFLOW) +" " + WORKFLOW_END + " "
            processed += utt.replace("wf-action:", WORKFLOW) +" " + WORKFLOW_END + " "
        elif "action:" in utt:
            processed += utt.replace("action:", ACTION) + " " + ACTION_END + " "

        # if i == len(utts) -1 and "system:" not in utt:
        #     processed
        
    if WORKFLOW in processed:
        #insert CONTEXT_END before first WORKFLOW
        processed = processed.split()
        index = processed.index(WORKFLOW)
        processed[index] = CONTEXT_END + " " + processed[index]
        processed = " ".join(processed)
    else:
        # insert CONTEXT_END before last system
        processed = processed.split()
        index = len(processed) -1 - processed[::-1].index(RESPONSE)
        processed[index] = CONTEXT_END + " " + processed[index]
        processed = " ".join(processed)

    # if "wf-action:" in procssed:
    #     processed = processed.replace("wf-action:", f"{CONTEXT_END} {WORKFLOW}")
    # else:

    # penult = utts[-2]
    # ult = utts[-1]

    # if "wf-action:" in penult:
    #     pass
    # else:
    #     pass
...
class DataTrainingArguments:
    ...
    dataset_type: Optional[str] = field(
        default="utt",
        metadata={"help": "The type of the dataset to use (via the datasets library)."},
    )


    dataset_name: Optional[str] = field(
        default=None, #"./data/wc_seed_one_convo.json",
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        def process(split):
            ...
            for d in split:
                ...
                #    else:
                #        res[k] += [v]
                #target = d["target"].split(":")[-1].strip()
                #REP_START
                #print(d)
                ex = example_process(data_args.dataset_type, d)
                #print(ex)
                #exit()
                res["text"] += [ ex ] # last \n is need bc of blocking
                #print(res["text"])
                #res["text"] += [ d["input"]+"\n"+"system: "+REP_START+target]
                #print(res["text"])
                #exit()   
            return res
    ...
    print("random train sample:", (raw_datasets["train"][rint]["text"]))
    # exit()
    ...
    new_inits = np.vstack([np.random.normal(loc=mean_weights, scale=std_weights) for _ in range(len(additional_special_tokens))])
    ...
    with torch.no_grad():
        ...
        # a = self.model.get_input_embeddings().weight
        # print(a)
        # print(a.shape)
        # print(new_inits)
        # print("new_shape:", new_inits.shape)
        model.get_input_embeddings().weight[-len(additional_special_tokens):, :] = new_inits
    ########################################
    ...
    with training_args.main_process_first(desc="dataset map tokenization"):
        ...
        print("Shuffling tokenized dataset")
        tokenized_datasets = tokenized_datasets.shuffle(seed=234)
    ...
    def pad_texts(examples):
        ...
        for input_ids, attention_mask in zip(examples['input_ids'],examples['attention_mask']):
            ...
            input_ids.extend([tokenizer.eos_token_id] * (block_size - len(input_ids)))
    ...
    if data_args.one_example_per_block:
        print('Padding texts')
    ...
    if data_args.mask_context:
        ...
        mask_context_torch_data_collator,
        rep_start_tokenid=tokenizer.encode(RESPONSE)[0],
