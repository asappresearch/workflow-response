...
"""
23/07/12 TODO
1. implement a way to grab equivalent block GT response, not just immediate
"""
...
def generate_tf_batch(context, model, tokenizer, num_responses, context_len, response_len, scorer=False):
    ...
    """
    TODO: need some logic here to cut where
    the workflow block has been ended
    ==> that is when action been done
    b2: possible (cause it has action)
    a1s: possible
    if no action then go until when context is finished
    processinf list: ACTION ==> CONTEXT
    """
...
def evaluate_model(
    ...
    do_batch: bool = True,
    batch_size: int = 16,
    cascade_datapath: str = "./test_results/230809/dist_st/wf_prediction_epochs10/epoch10/evaluation_tf.csv",
