...
output_paths = [ "./test_results/annotator_c.csv", "./test_results/annotator_a.csv" ,"./test_results/annotator_b.csv"]
output_paths =  [ "./block_internal_annotator_c.csv", "./block_internal_annotator_b.csv"]
KEY_SET = []
    ...
    with open(datapath, 'r') as data:
        ...
        for line in csv.DictReader(data):
            ...
            for r,s,k in zip(responses, scores,keys):
                res_dic[k] = {"response": r, "score":s, "context":context, "wf":true_wf, "subflow":subflow}
...
for k, v in eval_data.items():
    ...
    # predictions = [1 for x in prompts]#
    predictions = asyncio.run(batch_evaluate_reward(prompts, responses, workflows, subflows))
    eval_result[k] = predictions
...
for model, scores in eval_result.items():
    with open(f"./test_results/{model}.csv", "w") as fh:
        header1 = ["context", "subflow","workflow", "response","model_score", "annotator_scores", "gt_response", "average",  ]
        ...
        for i, score in enumerate(scores):
            ...
            #row["average"] = row["annotator_scores"][0] # np.average([x for x in row["annotator_scores"] if x!=-2])
