...
def convo_to_context_response_pairs_workflow_response(dataset_type: str, datum):
    ...
    if dataset_type == "b1":
        import re 
        def process(s):
            pattern = re.compile("<\|action\|>[^<]*<\|endofaction\|>") #re.compile(f"{ACTION}*{ACTION_END}")
            # THIS WAS THE CAUSE OF ERROR.... basically removing the outermost... action span... not what I intended
            #print(pattern)
            s = pattern.sub("", s)
            # split = s.split(RESPONSE)
            # split = [ pattern.sub("",s) for s in split]
            # s = "".join(split)
        ...
        def process(s):
            return s
        # if this works unify with this approach (re and reduce by case)
    split_strings = string.split(RESPONSE)        

    strings = []
    for i in range(len(split_strings)-1):
        #  < > RESPONSE ... RESPONSE
        #   0     ....  1     .....   2
        parts = split_strings[:i+1]
        context = RESPONSE.join(parts).strip() +RESPONSE
        response = split_strings[i+1].split(RESPONSE_END)[0].strip()
        response = response.replace(RESPONSE_END,"")

        remaining = []
        for ss in split_strings[i+1:]:
            res = ss.split(RESPONSE_END)[0].strip()
        ...
        block_responses = remaining
        dic = {"context": process(context), "response": response, "block_responses": block_responses }
        if PRINT:
            print(dic)
    ...
    #     response = response.replace(RESPONSE_END,"")
    #     """
    #     TODO: build the block responses here
    #           They are the system responses starting from response to next action
    #     """
    #     block_responses = 
    #     dic = {"context": context, "response": response, "block_responses": block_responses }
    #     if PRINT:
    #         print(dic)
    #     context_response_pairs.append(dic)
    #     if context.strip().endswith(USER_END):
...
def evaluate_model(
    ...
    with Pool() as p:
        if dataset == "abcd":
            ...
            for idx, pair in context_response_pairs.iterrows():
                ...
                block_responses = pair["block_responses"]
                ...
                dic = {
                    ...
                    "cont_block_responses": block_responses,
                    "block_responses": block_responses # not used just for convenience
                }
                new.append(dic)
                if PRINT:
                    print(dic) #
            ...
            with open(cascade_datapath, 'r') as data:
                for line in csv.DictReader(data):
                    ...
                    block_responses = line["cont_block_responses"]
                    #print("block responses", block_responses)
                    #print("block responses", type(block_responses))
                    ...
                    dic = { "context": context, "response": response, "true_wf": true_wf, "block_responses": block_responses }
                    if PRINT:
                        print(dic) #
                    pairs += [dic]
    ...
    if not do_batch:
        ...
        for idx, row in tqdm(context_response_pairs.iterrows(), total=context_response_pairs.shape[0]):
            ...
            block_responses = row["block_responses"]
            cr += [ {"context": context, "response": response, "block_responses": block_responses}]
        ...
        all_block_responses = list(chunk(all_block_responses, size=num_responses))
    ...
    blocks_flat = [p for pred_responses in all_block_responses for p in pred_responses]
    #print(blocks_flat)
    #exit()
    assert len(preds_flat) == len(gold_flat)
    assert len(preds_flat) == len(blocks_flat)

    if True:
        p,g = [], []
        b = []
        count = 0
        for pp,gg,bb in zip(preds_flat, gold_flat, blocks_flat):
            for stoken in SPECIAL_TOKEN_SET:
                pp = pp.replace(stoken, "").strip()
                gg = gg.replace(stoken, "").strip()
                bb = [bbb.replace(stoken, "").strip() for bbb in bb]    
    ...
    # TODO: just use thw following code, construt gold_flat and num_responses in the following way
    if dataset_type != "wf":
        max_num = -float("inf")
        for b in blocks_flat:
            #print(b)
            max_num = max(max_num, len(b))
        #print(blocks_flat)
        #print(max_num)
        #exit()
        num_responses = max_num

        new_gold_flat = []
        for b in blocks_flat:
            num_len = len(b)
            temp = b + ["" for x in range(num_responses-num_len)]
            new_gold_flat += temp
        gold_flat = new_gold_flat
        #preds_flat
        preds_flat = [gold_response for gold_response in preds_flat for i in range(num_responses)]
    ...
    for metric in metrics:
        ...
        if metric == 'bert_score':
            ...
            for pred, gold in zip(preds_flat, gold_flat):
                #pred = "\n"
                results = evaluate_metric.compute(predictions=[pred], references=[gold])[metric] if pred.strip() != "" and \
                gold.strip()!="" else 0.
    ...
    for cidx, (_, row) in enumerate(context_response_pairs.iterrows()):
        ...
        row_dict["block_respones"] = row["block_responses"]
        if "cont_response" in row:
            row_dict["cont_response"] = row["cont_response"]
        if "cont_block_responses" in row:
            row_dict["cont_block_responses"] = row["cont_block_responses"]
        if "true_wf" in row:
        ...
        for ridx in range(0, num_responses):
            ...
            row_dict[f'response_{ridx+1}'] = preds_flat[tidx] #all_pred_responses[cidx][ridx]
            tidx += 1
            for metric in metrics:
