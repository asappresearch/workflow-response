...
"""
datapath = "./test_results/block/230802/dist_st/utt_prediction_oracle_wf/epoch10/evaluation_tf.csv"
stat_data = []
...
for i in range(LEN):
    ...
    trigger = False
    for k, v in eval_data.items():
        response = "Agent: " + v[i]["response"]
        response = response.replace(USER, "\nClient: ")
        response = response.replace(RESPONSE, "\nAgent: ")
        response = response.replace(WORKFLOW, "\nNext Action: ")
        response = response.replace(ACTION, "\nAction: ")

        for stoken in SPECIAL_TOKEN_SET:
            response = response.replace(stoken, "")

        dic[k] = response #v[i]["response"]
        #dic["true_response"] = v[i]["true_response"]

        """
        preparing block gt response
        """
        #print(k, v[i]["true_response"])
        true_response =  v[i]["true_response"]
        true_response = "Agent: " + true_response
        true_response = true_response.replace(USER, "\nClient: ")
        true_response = true_response.replace(RESPONSE, "\nAgent: ")
        true_response = true_response.replace(WORKFLOW, "\nNext Action: ")
        true_response = true_response.replace(ACTION, "\nAction: ")

        for stoken in SPECIAL_TOKEN_SET:
            true_response = true_response.replace(stoken, "")
    ...
    if len(context.strip().split("Action:")[-1].split()) == 1:
        trigger = False
    else:
        trigger = True
...
with open("block_test.csv", "w") as fh:
    ...
    for i in range(LEN):
        ...
        row["keys"] = [ x.split("/")[4] if x!="true_response" else x for x in keys ]
