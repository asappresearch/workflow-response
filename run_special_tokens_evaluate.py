...
def convo_to_context_response_pairs_workflow_response(
    ...
    if True:
        ...
        if a.endswith("workflow"):
            ...
            try:
                # task is workflow prediction here
                # but this cannot handle multiple workflows, i.e. oracle
                # this actually works, since for future I set it workflow [list] 
                # so workflow occurs only once
                context = text.split(WORKFLOW)[0].strip() + " " + WORKFLOW
                response = text.split(WORKFLOW)[1].strip() #.split()[0]
            ...
            try:
                # task is utterance prediction
                context = text.split(RESPONSE)[0].strip() + " " + RESPONSE
                response = text.split(RESPONSE)[1].strip() #.split()[0]
            except:
                print("wot")    
                exit(1)
        else:
...
def evaluate_model(
    ...
    do_batch: bool = True,
    batch_size: int = 16,
    cascade_datapath: str = "./test_results/wf_prediction_e2e/evaluation_tf.csv", #"./test_results/wf_prediction/evaluation_tf.csv",
    ...
    with Pool() as p:
        if dataset == "abcd":
            ...
            func = partial(convo_to_context_response_pairs_workflow_response)
